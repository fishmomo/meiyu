...
class MaskStepTest(unittest.TestCase):
    ...
    def test_resolve_case_masks_rejects_front1(self) -> None:
        from pipeline.steps.masks import resolve_case_masks

        with self.assertRaisesRegex(ValueError, "front2"):
            resolve_case_masks("front1", "2017-06-22T18")

    def test_resolve_case_masks_raises_when_required_file_is_missing(self) -> None:
        from pipeline.steps.masks import resolve_case_masks

        missing_path = "H:\\fake\\missing_front2_extend.nc"

        def fake_exists(self: Path) -> bool:
            return str(self) != missing_path

        with patch("pipeline.core.mask_ops.cra40_front_extend", return_value=missing_path):
            with patch("pathlib.Path.exists", autospec=True, side_effect=fake_exists):
                with self.assertRaises(FileNotFoundError) as ctx:
                    resolve_case_masks("front2", "2017-06-22T18")
